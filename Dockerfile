...
RUN cd /ckb-rosetta-sdk; \
    git fetch --tags; \
    git checkout tags/v0.3.3 -b v0.3.3; \
    go mod download; \
    cd server; \
    go build .
...
COPY --from=builder /ckb-rosetta-docker-config/setup.sh /setup.sh
COPY --from=builder /ckb-rosetta-docker-config/Procfile /conf/Procfile
COPY --from=builder /ckb-rosetta-docker-config/config.yaml /config.yaml
